...
📂 **Hlavní soubory**
 ...
 - [echo_receiver.vhd](project_files/echo_receiver.vhd) – Tento modul slouží k měření vzdálenosti na základě doby trvání signálu ```echo_in```, přičemž po obdržení impulsu ```trig``` začne počítat počet hodinových cyklů (*clock cycles*) během logické jedničky na ```echo_in```, převede je na centimetry pomocí konstanty ```ONE_CM``` a výsledek poskytne na výstupu ```distance``` spolu s indikací platnosti měření pomocí signálu ```status```.
...
📂 **Main Files**
 ...
 - [echo_receiver.vhd](project_files/echo_receiver.vhd) – This module measures distance based on the duration of the ```echo_in``` signal. After receiving a ```trig``` pulse, it starts counting clock cycles during the logical high on ```echo_in```, converts the result to centimeters using the ```ONE_CM``` constant, and provides the distance on the output ```distance``` along with a validity indication via the ```status``` signal.
